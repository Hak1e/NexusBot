...
def generate_initial_embed_message(member):
    return discord.Embed(
        title="Добро пожаловать в комнату",
...
class MembersSelectMenu(discord.ui.Select):
    def __init__(self, members, pool, bot, action=ChannelActions.kick):
        options = [
    ...
    async def callback(self, ctx: discord.MessageInteraction):
        ...
        if self.action == ChannelActions.unban:
            for _id in selected_members_or_roles_ids:
                member_or_role = ctx.guild.get_member(int(_id)) or ctx.guild.get_role(
                    int(_id)
                )
...
class BaseDashboardButtons(discord.ui.View):
    ...
    async def create_member_select_menus(
        self, ctx, members, action=ChannelActions.kick
    ):
        menus = []
        for position in range(0, len(members), MAX_ITEMS_IN_MENU):
            menu = MembersSelectMenu(
                members[position : position + MAX_ITEMS_IN_MENU],
                action=action,
                pool=self.pool,
                bot=self.bot,
            )
    ...
    async def kick_from_room(
        ...
        if not await self.is_channel_author(ctx):
            await ctx.respond(
                "Вы можете использовать кнопки только в своём канале", ephemeral=True
            )
            return
    ...
    async def ban_in_room(
        ...
        if not members:
            await ctx.respond("В канале никого, кроме Вас", ephemeral=True)
            return
        await self.create_member_select_menus(ctx, members, ChannelActions.ban)

    @discord.ui.button(label="Разбанить", style=discord.ButtonStyle.blurple)
    async def unban_in_room(
        self, button: discord.ui.Button, ctx: discord.MessageInteraction
    ):
        if not await self.is_channel_author(ctx):
            await ctx.respond(
        ...
        if not members:
            ...
            return
        await self.create_member_select_menus(ctx, members, ChannelActions.unban)

    @discord.ui.button(label="Лимит", style=discord.ButtonStyle.blurple)
    async def change_room_limit(
        self, button: discord.ui.Button, ctx: discord.MessageInteraction
    ):
        if not await self.is_channel_author(ctx):
            await ctx.respond(
...
class ModalWindow(discord.ui.Modal):
    def __init__(self, ctx, pool, placeholder_text, text_input_title, action, bot):
        self.pool = pool
        self.action = action
        self.author_settings = AuthorSettings(bot)
        self.lobby_settings = LobbyChannelSettings(bot)
        text_input = discord.ui.TextInput(
            ...
            max_length=50,
            custom_id=f"text_input-{ctx.id}",
            placeholder=placeholder_text,
        )
        super().__init__(
            title=text_input_title, custom_id=f"modal-{ctx.id}", components=text_input
        )
    ...
    async def callback(self, ctx: discord.Interaction):
        ...
        if self.action == ChannelActions.edit_name:
            ...
            try:
                ...
                await ctx.respond(
                    embed=discord.Embed(
                        description=f"Битрейт канала успешно изменен на: `{value}`",
                        color=discord.Color.green(),
                    ),
                    ephemeral=True,
                )
                ...
                await ctx.respond(
                    embed=discord.Embed(
                        description="Введено неверное значение битрейта",
            ...
            if 0 <= int(value) <= 99:
                ...
                await ctx.respond(
                    embed=discord.Embed(
                        description=f"Лимит пользователей успешно изменен на: `{value}`",
                        color=discord.Color.green(),
                    ),
                    ephemeral=True,
                )
                ...
                if message:
                    await self.lobby_settings.update_lobby_info_message(
                        message, ctx.channel
                    )
                ...
                await ctx.respond(
                    embed=discord.Embed(
                        description="Введено неверное значение лимита пользователей.\n"
...
class CustomChannelDashboardButtons(BaseDashboardButtons):
    ...
    async def change_room_visibility(
        self, button: discord.ui.Button, ctx: discord.MessageInteraction
    ):
        if not await self.is_channel_author(ctx):
            await ctx.respond(
        ...
        current_view_channel_permission = ctx.channel.permissions_for(
            default_role
        ).view_channel
        await ctx.channel.set_permissions(
            default_role, view_channel=not current_view_channel_permission
        )
        await ctx.respond(
            (
                "Комната скрыта"
                if current_view_channel_permission is not False
                else "Комната больше не скрыта"
            ),
            ephemeral=True,
        )
        assert isinstance(ctx.channel, discord.VoiceChannel)
    ...
    async def change_room_access(
        ...
        await ctx.channel.set_permissions(
            default_role, connect=not current_connect_permission
        )
        assert isinstance(ctx.channel, discord.VoiceChannel)
        await self.author_settings.update_voice_channel_overwrites(ctx.channel)
        await ctx.respond(
            (
                "Комната закрыта"
                if current_connect_permission is not False
                else "Комната больше не закрыта"
            ),
            ephemeral=True,
        )
    ...
    async def change_room_bitrate(
        ...
        modal_window = ModalWindow(
            ctx,
            self.pool,
            f"8 - {int(ctx.guild.bitrate_limit / 1000)}",
            "Введите битрейт канала",
...
class Lobby(commands.Cog):
    ...
    @staticmethod
    def create_lobby_info_embed(
        member, role: discord.Role, voice_channel: discord.VoiceChannel, user_limit
    ):
    ...
    async def create_voice_channel(
        ...
        if custom:
            ...
            if isinstance(required_role, discord.Role):
                query = (
                    "SELECT channel_with_role_prefix "
                ...
                query = (
                    "SELECT channel_without_role_prefix "
                    "FROM lobby_voice_channel_creator_settings "
                    "WHERE id = $1"
                )
                prefix = await self.pool.fetchval(query, voice_creator.id)
    ...
    async def on_voice_state_update(
        ...
        if before.channel:
            ...
            voice_creator_id = await self.lobby_settings.get_channel_creator_id(
                before.channel.id
            )
            if voice_creator_id:
                ...
                if before.channel.id in self.cached_messages:
                    ...
                    message = await self.lobby_settings.get_lobby_info_message(
                        before.channel
                    )
                    if message:
                ...
                if not before.channel.members:
                        ...
                        if message.id not in self.queued_message_id:
                            ...
                            while True:
                                ...
                                try:
                                    ...
                                    await self.lobby_settings.delete_message_id_from_db(
                                        message.id
                                    )
                                    logging.info(
                                        "Lobby info message deleted from database"
                                    )
                                    self.queued_message_id.remove(message.id)
                                    break
                                except Exception as e:
                                    logging.error(
                                        f"[{counter}] Error while deleting message: {e}"
                                    )
                                    counter += 1
                                    await asyncio.sleep(1)
                    else:
                    ...
                    while True:
                        ...
                        try:
                            logging.info(
                                f"Deleting voice channel ({before.channel.name})"
                            )
                            await before.channel.delete()
                            logging.info(
                                f"Voice channel ({before.channel.name}) deleted"
                            )
                            await self.lobby_settings.delete_voice_channel_author_id(
                                before.channel
                            )
                            logging.info("Deleted voice channel author from database")
                            await self.lobby_settings.delete_created_voice_channel_from_db(
                                before.channel
                            )
                    ...
                    logging.info(
                        f"Before channel ({before.channel.name}) is not empty. Updating lobby info"
                    )
                    if message:
                        await self.lobby_settings.update_lobby_info_message(
        ...
        if current.channel:
            ...
            if is_voice_creator:
                ...
                if voice_creator_id:
                    ...
                    query = (
                        "SELECT category_id_for_new_channel "
                        "FROM lobby_voice_channel_creator_settings "
                        "WHERE id = $1"
                    )
                    lobby_category_id = await self.pool.fetchval(
                        query, current.channel.id
                    )
                    if not lobby_category_id:
                        logging.error(
                            f"Category for new channel was not found in database"
                        )
                        return
                    category = member.guild.get_channel(lobby_category_id)
                    overwrites = await self.lobby_settings.get_channel_overwrites(
                        category, member
                    )
                    hello_embed = generate_initial_embed_message(member)
                    if custom:
                        channel_name, bitrate, user_limit = (
                            await self.lobby_settings.get_custom_channel_settings(
                                guild_id, member
                            )
                        )
                        voice_channel = await self.create_voice_channel(
                            member=member,
                            category=category,
                            overwrites=overwrites,
                            custom=True,
                        ...
                        try:
                            ...
                            await self.lobby_settings.add_lobby_channel_to_db(
                                voice_channel.id, voice_creator_id
                            )
                            logging.info("Voice channel added to database")
                            await self.lobby_settings.set_voice_channel_author_id(
                        ...
                        required_role = (
                            await self.lobby_settings.get_channel_required_role(
                                member, current.channel.id
                            )
                        )
                        query = (
                            "SELECT user_limit "
                        ...
                        try:
                            ...
                            await voice_channel.delete()
                            logging.error(
                                f"Voice channel ({voice_channel.name}) deleted"
                            )
                            return
                        # await asyncio.sleep(1)
                        if voice_channel.members:
                            if required_role == RequestedRole.missing:
                                query = (
                                    "SELECT role_not_found_message "
                                    "FROM lobby_voice_channel_creator_settings "
                                    "WHERE id = $1"
                                )
                                role_not_found_message = await self.pool.fetchval(
                                    query, voice_creator_id
                                )
                                ...
                                error_message = (
                                    role_not_found_message or default_error_message
                                )
                                embed = discord.Embed(
                            ...
                            embed = self.create_lobby_info_embed(
                                member,
                                required_role,
                                voice_channel,
                            ...
                            lobby_log_needed = await self.lobby_settings.log_needed(
                                voice_creator_id
                            )
                            if lobby_log_needed:
                                logging.info("Lobby info needed")
                                text_channel_id = (
                                    await self.lobby_settings.get_text_channel_id(
                                        voice_creator_id
                                    )
                                )
                                ...
                                if not text_channel:
                                    ...
                                    logging.info(
                                        "Text channel for lobby info found. Sending message"
                                    )
                                    lobby_info_message = await text_channel.send(
                                        embed=embed
                                    )
                                    logging.info("Message sent")
                                    self.cached_messages[voice_channel.id] = (
                                        lobby_info_message
                                    )
                                    await self.lobby_settings.save_message_id_to_db(
                                        voice_channel, lobby_info_message
                                    )
                                    logging.info(
                                        "Sent message cached and saved to database"
                                    )
                ...
                if not voice_creator_id:
                    return
                logging.info(
                    f"{member.name} joined lobby room ({current.channel.name})"
                )
