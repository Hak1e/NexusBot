...
class Punishments(commands.Cog):
    ...
    async def mute(
        ...
        """Замьютить участника"""
        query = "SELECT id " "FROM guild_mute_role " "WHERE guild_id = $1"
        muted_role_id = await self.pool.fetchval(query, ctx.guild.id)
        if muted_role_id is None:
            return await ctx.send(
                "Роль мьюта не настроена на этом сервере. Обратитесь к администратору",
                ephemeral=True,
            )
        muted_role = ctx.guild.get_role(muted_role_id)
        if not muted_role:
            return await ctx.send(
                "Роль мьюта не найдена. Обратитесь к администратору", ephemeral=True
            )
