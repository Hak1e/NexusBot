...
class Tournament(commands.Cog):
    ...
    async def add(self, ctx: discord.ApplicationContext,
                  member_or_id: discord.User, reason: str):
        ...
        await ctx.respond(f"{member_or_id.mention} добавлен в чёрный список", ephemeral=True)

    @blacklist.command()
    async def remove(self, ctx: discord.ApplicationContext,
                     member_or_id: discord.User):
        ...
        await self.pool.execute(query, ctx.guild.id,
                                member_or_id.id)
        await ctx.respond(f"{member_or_id.name} удалён из чёрного списка", ephemeral=True)
    ...
    async def show(self, ctx: discord.ApplicationContext,
        ...
        if not blacklist:
            empty_embed = discord.Embed(title="Чёрный список турнира", description="Список пуст")
        ...
        await ctx.respond(embed=pages[0], view=buttons,
                       ephemeral=ephemeral)
