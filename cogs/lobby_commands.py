...
class ChannelCommands(commands.Cog):
    ...
    async def is_channel_author(self, ctx):
        if await self.author_settings.get_voice_channel_author_id(ctx.channel) != ctx.author.id:
            await ctx.respond("Вы можете использовать команды только в своём канале", ephemeral=True)
            return False
        return True
    ...
    async def dashboard(self, ctx: discord.ApplicationContext):
        ...
        if not isinstance(ctx.channel, discord.VoiceChannel):
            return await ctx.respond("Используйте команду в своём голосовом канале", ephemeral=True)
        if not await self.is_channel_author(ctx):
            return await ctx.respond("Вы можете использовать команды только в своём канале", ephemeral=True)
    ...
    async def vc_kick(self, ctx: discord.ApplicationContext,
    ...
    async def vc_ban(self, ctx,
        ...
        await ctx.respond(f"{member.mention} забанен в Вашем голосовом канале", ephemeral=True)
        if member in ctx.channel.members:
            await member.move_to(None)  # type: ignore
    ...
    async def vc_unban(self, ctx):
        ...
        if not members:
            return await ctx.respond("В настройках канала нет заблокированных участников", ephemeral=True)
    ...
    async def visible(self, ctx,
        ...
        await self.author_settings.update_voice_channel_overwrites(ctx.channel)
        await ctx.respond("Комната скрыта" if value is True else "Комната больше не скрыта", ephemeral=True)
    ...
    async def connect(self, ctx,
        ...
        assert isinstance(ctx.channel, discord.VoiceChannel)
        await self.author_settings.update_voice_channel_overwrites(ctx.channel)
        await ctx.respond("Комната закрыта" if value is True else "Комната больше не закрыта", ephemeral=True)
    ...
    async def bitrate(self, ctx,
        ...
        await ctx.respond(embed=discord.Embed(description=f"Битрейт канала успешно изменён на: `{value}`",
                                           color=discord.Color.green()), ephemeral=True)
    ...
    async def limit(self, ctx,
        ...
        await ctx.respond(embed=discord.Embed(description=f"Лимит канала успешно изменён на: `{value}`",
                                           color=discord.Color.green()), ephemeral=True)
